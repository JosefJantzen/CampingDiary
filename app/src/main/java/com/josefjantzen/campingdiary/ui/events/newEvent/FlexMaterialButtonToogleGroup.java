...
public class FlexMaterialButtonToogleGroup extends FlexboxLayout {

    private static final String LOG_TAG = FlexMaterialButtonToogleGroup.class.getSimpleName();
    private static final int DEF_STYLE_RES =
            R.style.Widget_MaterialComponents_MaterialButtonToggleGroup;
    private final List<FlexMaterialButtonToogleGroup.CornerData> originalCornerData = new ArrayList<>();
    private final FlexMaterialButtonToogleGroup.CheckedStateTracker checkedStateTracker = new FlexMaterialButtonToogleGroup.CheckedStateTracker();
    //private final FlexMaterialButtonToogleGroup.PressedStateTracker pressedStateTracker = new FlexMaterialButtonToogleGroup.PressedStateTracker();
    private final LinkedHashSet<FlexMaterialButtonToogleGroup.OnButtonCheckedListener> onButtonCheckedListeners =
            new LinkedHashSet<>();
    ...
    public FlexMaterialButtonToogleGroup(@NonNull Context context) {
        this(context, null);
    }

    public FlexMaterialButtonToogleGroup(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.materialButtonToggleGroupStyle);
    }

    public FlexMaterialButtonToogleGroup(
    ...
    private static void updateBuilderWithCornerData(
            ShapeAppearanceModel.Builder shapeAppearanceModelBuilder, @Nullable FlexMaterialButtonToogleGroup.CornerData cornerData) {
        if (cornerData == null) {
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        originalCornerData.add(
                new FlexMaterialButtonToogleGroup.CornerData(
    ...
    public CharSequence getAccessibilityClassName() {
        return FlexMaterialButtonToogleGroup.class.getName();
    }
     ...
     * FlexMaterialButtonToogleGroup is in {@link #isSingleSelection() single selection mode}, then all
     ...
     * group changes. See {@link FlexMaterialButtonToogleGroup.OnButtonCheckedListener}.
     *
     * <p>Components that add a listener should take care to remove it when finished via {@link
     * #removeOnButtonCheckedListener(FlexMaterialButtonToogleGroup.OnButtonCheckedListener)}.
     *
     * @param listener listener to add
     */
    public void addOnButtonCheckedListener(@NonNull FlexMaterialButtonToogleGroup.OnButtonCheckedListener listener) {
        onButtonCheckedListeners.add(listener);
    }

    /**
     * Remove a listener that was previously added via {@link
     * #addOnButtonCheckedListener(FlexMaterialButtonToogleGroup.OnButtonCheckedListener)}.
     *
     * @param listener listener to remove
     */
    public void removeOnButtonCheckedListener(@NonNull FlexMaterialButtonToogleGroup.OnButtonCheckedListener listener) {
        onButtonCheckedListeners.remove(listener);
    }

    /** Remove all previously added {@link FlexMaterialButtonToogleGroup.OnButtonCheckedListener}s. */
    ...
    void updateChildShapes() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            FlexMaterialButtonToogleGroup.CornerData newCornerData = getNewCornerData(i, firstVisibleChildIndex, lastVisibleChildIndex);
    ...
    @Nullable
    private FlexMaterialButtonToogleGroup.CornerData getNewCornerData(
            int index, int firstVisibleChildIndex, int lastVisibleChildIndex) {
        FlexMaterialButtonToogleGroup.CornerData cornerData = originalCornerData.get(index);
        ...
        if (index == firstVisibleChildIndex) {
            return isHorizontal ? FlexMaterialButtonToogleGroup.CornerData.start(cornerData, this) : FlexMaterialButtonToogleGroup.CornerData.top(cornerData);
        }

        if (index == lastVisibleChildIndex) {
            return isHorizontal ? FlexMaterialButtonToogleGroup.CornerData.end(cornerData, this) : FlexMaterialButtonToogleGroup.CornerData.bottom(cornerData);
        }*/
    ...
    private void dispatchOnButtonChecked(@IdRes int buttonId, boolean checked) {
        for (FlexMaterialButtonToogleGroup.OnButtonCheckedListener listener : onButtonCheckedListeners) {
    ...
    public interface OnButtonCheckedListener {
         ...
         */
        void onButtonChecked(FlexMaterialButtonToogleGroup group, @IdRes int checkedId, boolean isChecked);
    }
    ...
    private static class CornerData {
        ...
        /*public static FlexMaterialButtonToogleGroup.CornerData start(FlexMaterialButtonToogleGroup.CornerData orig, View view) {
            return ViewUtils.isLayoutRtl(view) ? right(orig) : left(orig);
        }

        /** Keep the end side of the corner original data */
        /*public static FlexMaterialButtonToogleGroup.CornerData end(FlexMaterialButtonToogleGroup.CornerData orig, View view) {
            return ViewUtils.isLayoutRtl(view) ? left(orig) : right(orig);
        }

        /** Keep the left side of the corner original data */
        public static FlexMaterialButtonToogleGroup.CornerData left(FlexMaterialButtonToogleGroup.CornerData orig) {
            return new FlexMaterialButtonToogleGroup.CornerData(orig.topLeft, orig.bottomLeft, noCorner, noCorner);
        }

        /** Keep the right side of the corner original data */
        public static FlexMaterialButtonToogleGroup.CornerData right(FlexMaterialButtonToogleGroup.CornerData orig) {
            return new FlexMaterialButtonToogleGroup.CornerData(noCorner, noCorner, orig.topRight, orig.bottomRight);
        }

        /** Keep the top side of the corner original data */
        public static FlexMaterialButtonToogleGroup.CornerData top(FlexMaterialButtonToogleGroup.CornerData orig) {
            return new FlexMaterialButtonToogleGroup.CornerData(orig.topLeft, noCorner, orig.topRight, noCorner);
        }

        /** Keep the bottom side of the corner original data */
        public static FlexMaterialButtonToogleGroup.CornerData bottom(FlexMaterialButtonToogleGroup.CornerData orig) {
            return new FlexMaterialButtonToogleGroup.CornerData(noCorner, orig.bottomLeft, noCorner, orig.bottomRight);
        }
    }
